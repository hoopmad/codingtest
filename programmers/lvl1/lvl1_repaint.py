...
n = 100
m = 3
section = [7, 8, 9, 14, 16, 50, 51]

# 기존 풀이
def solution_old(n, m, section):
    distance = section[-1] - section[0] +1
    if distance <=  m:
        return 1
    else:
        return distance//m if distance%m==0 else distance//m + 1

# deque 사용 풀이
def solution(n, m, section):
    p_section = deque(section)
    paint = 0

    while(p_section):
        start = p_section.popleft()

        while(p_section and start + m > p_section[0]):
            p_section.popleft()
        paint += 1
